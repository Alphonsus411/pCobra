...
def visit_funcion(self, nodo: NodoFuncion):
    ...
    self.agregar_linea(f"fn {nodo.nombre}({params}) {{")
    self.indent += 1
...
class TranspiladorMojo(BaseTranspiler):
    """Transpila el AST de Cobra a un Mojo muy básico."""
...
for nombre, funcion in mojo_nodes.items():
    setattr(TranspiladorMojo, f"visit_{nombre}", funcion)
