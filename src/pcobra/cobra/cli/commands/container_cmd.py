...
def validar_tag(valor: str) -> str:
    ...
    if not re.match(r"^[a-zA-Z0-9_.-]+$", valor):
        raise ValueError(
            _("El tag debe contener solo letras, números, guiones y puntos")
        )
    return valor
...
class ContainerCommand(BaseCommand):
    ...
    def register_subparser(self, subparsers: Any) -> CustomArgumentParser:
        ...
        parser.add_argument(
            "--tag", default="cobra", help=_("Nombre de la imagen"), type=validar_tag
        )
    ...
    def run(self, args: Any) -> int:
        ...
        try:
            ...
            subprocess.run(
                ["docker", "build", "-t", args.tag, str(raiz)],
                check=True,
            ...
            mostrar_error(
                _("Error construyendo la imagen Docker: {err}").format(
            ...
            mostrar_error(
                _("Tiempo de espera agotado (10 minutos) al construir la imagen")
            )
            return 1
