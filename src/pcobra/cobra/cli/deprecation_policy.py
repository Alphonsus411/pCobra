...
def is_legacy_targets_mode(args: Namespace | object | None) -> bool:
    ...
    arg_enabled = (
        bool(getattr(args, "legacy_targets", False)) if args is not None else False
    )
...
def visible_public_targets(targets: tuple[str, ...] | list[str]) -> tuple[str, ...]:
    ...
    return tuple(
        target for target in targets if target not in DEPRECATED_PUBLIC_TARGETS
    )


def emit_target_deprecation_event(
    *, command: str, target: str, phase: int, legacy_mode: bool
) -> None:
...
def enforce_target_deprecation_policy(
    *, command: str, target: str, args: Namespace | object | None
) -> None:
    ...
    emit_target_deprecation_event(
        command=command, target=canonical, phase=phase, legacy_mode=legacy_mode
    )
...
def enforce_advanced_profile_policy(
    *, command: str, args: Namespace | object | None
) -> None:
    ...
    if phase < 2:
        if profile != "avanzado":
            mostrar_advertencia(
                _(
                    "El comando '{command}' expone capacidades avanzadas de backend; use --perfil avanzado."
