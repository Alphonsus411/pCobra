...
Motor real de sugerencias: Cobra mantiene `agix` en `[project].dependencies` como dependencia oficial de la distribución y como único nombre canónico para esta integración; no es un extra opcional. Una instalación parcial o un entorno headless puede no tener el paquete importable, situación que Cobra tolera mientras no se soliciten sugerencias: al solicitarlas informa claramente cómo instalar la dependencia, sin impedir otros flujos. Cualquier cambio de motor requiere una ADR aprobada que justifique el cambio de arquitectura. La fachada pública estable es `pcobra.ia.analizador_sugerencias`, que delega en `pcobra.ia.analizador_agix` y conserva el contrato de validación previa y posterior con `Lexer(codigo).tokenizar()` y `Parser(tokens).parsear()` antes de exponer sugerencias aplicables.

La salida visible de GUI/IA separa dos planos. **Errores de Lexer/Parser** son bloqueantes: describen texto que Cobra no puede tokenizar o parsear, por lo que la GUI solo muestra el diagnóstico y pide corregirlo antes de sugerir cambios. En ese caso, ni **Sugerencias del Libro** ni **Corrección** invocan el motor IA. **Sugerencias del Libro** es una vista de recomendaciones pedagógicas no bloqueantes: solo aparece tras una validación léxica/sintáctica correcta con `Lexer(codigo).tokenizar()` y `Parser(tokens).parsear()`, y agrupa sus resultados por categoría (`léxico/sintaxis`, `estilo`, `nombres`, `forma canónica` y `observabilidad`) usando los metadatos estructurados de cada regla (`categoria`, `severidad` y `aplicable_automaticamente`). **Corrección** genera un reporte tipográfico/estilístico no destructivo para revisión humana, también sujeto a la misma validación previa. Ninguna de estas acciones modifica automáticamente el editor.
...
*   **Gestión de archivos:**
    ...
    *   **Árbol de directorios:** Usa la API canónica `pcobra.gui.runtime.crear_arbol_directorios`. El panel reconstruye el primer nivel visible cuando cambia la raíz, cuando se abre un archivo y cuando se guarda, de modo que la vista refleje la carpeta base y el archivo activo sin recorrer todo el árbol por adelantado. En esa primera reconstrucción muestra primero las carpetas y después los archivos reconocidos en orden estable. Los fuentes Cobra cargables usan la extensión `.cobra`; la extensión `.co` está reservada para paquetes y no se carga como código fuente. Las subcarpetas no se precargan: se cargan bajo demanda al expandirse mediante `crear_arbol_directorios()`. Solo los archivos fuente Cobra visibles disparan la carga en el editor; el IDLE conecta el evento `on_click` con `cargar_archivo_desde_arbol`, que valida la extensión antes de leer el contenido y actualiza el estado del archivo activo. El campo `Raíz del árbol` y el botón `Establecer raíz` permiten cambiar la carpeta base del panel sin depender de Lexer/Parser: el valor introducido pasa por `normalizar_ruta_archivo_gui()`, respeta el sandbox compartido con `pcobra.corelibs.archivo`, se rechaza con un mensaje claro si no es un directorio y, al aceptarse, actualiza `directorio_actual` y reconstruye el primer nivel visible. Esta funcionalidad pertenece al manejo de archivos/rutas de la GUI y no cambia la sintaxis Cobra, por lo que no requiere modificar `Lexer` ni `Parser`.
...
*   **Sugerencias de código:**
    *   **Sugerencias del Libro:** Muestra recomendaciones pedagógicas agrupadas por categoría y requiere que la dependencia oficial `agix` (nombre canónico del motor) esté disponible. Al abrir el IDLE, la GUI detecta de forma liviana si el motor se puede importar sin ejecutar análisis ni cargar la integración completa; si falta en una instalación parcial o headless, el botón queda deshabilitado y muestra un tooltip explicativo para que el resto del entorno siga funcionando. Cuando `agix` está disponible, la acción valida primero el contenido del editor con `Lexer(codigo).tokenizar()` y `Parser(tokens).parsear()`. Si el código no supera esa validación léxica/sintáctica, informa el diagnóstico, no invoca el motor IA y no inventa correcciones. Si el código es válido, solicita sugerencias mediante la integración de `agix`, pero los candidatos se construyen desde las reglas trazables del Libro definidas en `src/pcobra/ia/reglas_libro_programacion.py`; por eso cada detalle conserva metadatos como `rule_id`, `rule_section`, `category`, `severity` y `automatic`, y cada fragmento propuesto vuelve a validarse con Lexer/Parser antes de mostrarse en la GUI. La salida del panel mantiene una sección de errores léxicos/sintácticos y, cuando procede, una sección de sugerencias agrupadas por `léxico/sintaxis`, `estilo`, `nombres`, `forma canónica` y `observabilidad`. Esta acción no modifica automáticamente el editor.
    *   **Corrección:** Genera un reporte tipográfico/estilístico no destructivo para revisión humana. Usa la misma validación previa obligatoria que Sugerencias del Libro: primero ejecuta `Lexer(codigo).tokenizar()` y después `Parser(tokens).parsear()`. Si aparece un error léxico o sintáctico, muestra el diagnóstico, no invoca el motor IA y deja intacto el editor. Solo con código válido consulta la integración oficial, si está disponible, y presenta el reporte en la salida; el usuario decide manualmente si aplica algún cambio.
