...
La salida visible de GUI/IA separa dos planos. **Errores de Lexer/Parser** son bloqueantes: describen texto que Cobra no puede tokenizar o parsear, por lo que la GUI solo muestra el diagnóstico y pide corregirlo antes de sugerir cambios. **Sugerencias estilísticas del Libro** son no bloqueantes: solo aparecen tras una validación léxica/sintáctica correcta y se agrupan por tipo (`léxico/sintaxis`, `estilo`, `nombres`, `forma canónica` y `observabilidad`) usando los metadatos estructurados de cada regla (`categoria`, `severidad` y `aplicable_automaticamente`).
...
*   **Sugerencias de código:**
    *   **Sugerencias del Libro:** Herramienta de corrección tipográfica/estilística que requiere tener configurada la dependencia IA opcional `agix` (nombre canónico del motor). Al abrir el IDLE, la GUI detecta de forma liviana si el motor está disponible sin ejecutar análisis ni cargar la integración completa; si falta, el botón queda deshabilitado y muestra un tooltip explicativo para que el resto del entorno siga funcionando. Cuando `agix` está instalado, la acción valida primero el contenido del editor con `Lexer(codigo).tokenizar()` y `Parser(tokens).parsear()`. Si el código no supera esa validación léxica/sintáctica, informa el error y no inventa correcciones. Si el código es válido, solicita sugerencias mediante la integración opcional de `agix`, pero los candidatos se construyen desde las reglas trazables del Libro definidas en `src/pcobra/ia/reglas_libro_programacion.py`; por eso cada detalle conserva metadatos como `rule_id`, `rule_section`, `category`, `severity` y `automatic`, y cada fragmento propuesto vuelve a validarse con Lexer/Parser antes de mostrarse en la GUI. La salida del panel mantiene una sección de errores léxicos/sintácticos y, cuando procede, una sección de sugerencias agrupadas por `léxico/sintaxis`, `estilo`, `nombres`, `forma canónica` y `observabilidad`.
