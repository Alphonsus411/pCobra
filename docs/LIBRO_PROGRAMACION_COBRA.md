...
Motor real de sugerencias: Cobra mantiene `agix` como dependencia oficial y único nombre canónico para esta integración. `agi-core` no es una dependencia declarada del proyecto ni un motor alternativo soportado; si aparece en discusiones o documentación externa debe tratarse como nombre erróneo/alternativo y normalizarse a `agix`. No debe añadirse una integración paralela con otro motor sin una ADR aprobada que justifique el cambio de arquitectura. La fachada pública estable es `pcobra.ia.analizador_sugerencias`, que delega en `pcobra.ia.analizador_agix` y conserva el contrato de validación previa y posterior con `Lexer(codigo).tokenizar()` y `Parser(tokens).parsear()` antes de exponer sugerencias aplicables.
