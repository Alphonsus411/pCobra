...
*   **Sugerencias de código:**
    *   **Sugerencias del Libro:** Muestra recomendaciones pedagógicas agrupadas por categoría y requiere tener configurada la dependencia IA opcional `agix` (nombre canónico del motor). Al abrir el IDLE, la GUI detecta de forma liviana si el motor está disponible sin ejecutar análisis ni cargar la integración completa; si falta, el botón queda deshabilitado y muestra un tooltip explicativo para que el resto del entorno siga funcionando. Cuando `agix` está instalado, la acción valida primero el contenido del editor con `Lexer(codigo).tokenizar()` y `Parser(tokens).parsear()`. Si el código no supera esa validación léxica/sintáctica, informa el diagnóstico, no invoca el motor IA y no inventa correcciones. Si el código es válido, solicita sugerencias mediante la integración opcional de `agix`, pero los candidatos se construyen desde las reglas trazables del Libro definidas en `src/pcobra/ia/reglas_libro_programacion.py`; por eso cada detalle conserva metadatos como `rule_id`, `rule_section`, `category`, `severity` y `automatic`, y cada fragmento propuesto vuelve a validarse con Lexer/Parser antes de mostrarse en la GUI. La salida del panel mantiene una sección de errores léxicos/sintácticos y, cuando procede, una sección de sugerencias agrupadas por `léxico/sintaxis`, `estilo`, `nombres`, `forma canónica` y `observabilidad`. Esta acción no modifica automáticamente el editor.
    *   **Corrección:** Genera un reporte tipográfico/estilístico no destructivo para revisión humana. Usa la misma validación previa obligatoria que Sugerencias del Libro: primero ejecuta `Lexer(codigo).tokenizar()` y después `Parser(tokens).parsear()`. Si aparece un error léxico o sintáctico, muestra el diagnóstico, no invoca el motor IA y deja intacto el editor. Solo con código válido consulta la integración opcional y presenta el reporte en la salida; el usuario decide manualmente si aplica algún cambio.
