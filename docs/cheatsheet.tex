...
\texttt{tomar\_mientras(lista, funcion)} & Prefijo mientras la condición sea cierta; equivalente al patrón de recorrido perezoso en la librería estándar de Cobra.\\
\texttt{descartar\_mientras(lista, funcion)} & Omite el prefijo que cumple la condición, siguiendo la misma semántica de filtros progresivos de Cobra.\\
\texttt{scanear(lista, funcion, inicial)} & Acumulaciones parciales para construir estados intermedios sobre secuencias Cobra.\\
\texttt{pares\_consecutivos(lista)} & Pares deslizantes para análisis local de vecindad dentro de listas Cobra.\\
...
\texttt{subcadena\_antes(cad, sep, defecto)} & Prefijo antes del primer separador con semántica consistente entre los targets oficiales.\\
...
\begin{tabular}{ll}
\textbf{Función} & \textbf{Descripción}\\\hline
\texttt{ordenar\_tabla(datos, por, ascendente)} & Ordena múltiples columnas sobre listas de registros manteniendo el contrato estable de Cobra.\\
