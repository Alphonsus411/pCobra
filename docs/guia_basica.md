...
Los ejemplos de **Hello World** se encuentran en la carpeta [`examples/hello_world`](../examples/hello_world). Para UX pública enfócate en `python`, `rust` y `javascript`; los demás targets quedan reservados para migración interna/regresión.

Para interpretar correctamente esos ejemplos:

- **Transpilación pública**: `python`, `rust`, `javascript`.
- **Targets legacy internos (migración/regresión)**: `go`, `cpp`, `java`, `wasm`, `asm`.
