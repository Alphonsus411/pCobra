...
Los ejemplos de **Hello World** se encuentran en la carpeta [`examples/hello_world`](../examples/hello_world) con un subdirectorio por target oficial. En esa misma carpeta hay un `README.md` que muestra cómo transpilar cada archivo `.co` a su lenguaje destino y contiene los resultados pre-generados. La lista canónica completa es: `python`, `rust`, `javascript`, `wasm`, `go`, `cpp`, `java` y `asm`.

Para interpretar correctamente esos ejemplos:
