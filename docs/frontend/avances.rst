...
- **Transpilacion a otros lenguajes**: Se ha implementado un transpilador que convierte el codigo Cobra a los 8 targets oficiales con nombres canónicos: ``python``, ``rust``, ``javascript``, ``wasm``, ``go``, ``cpp``, ``java`` y ``asm``.
