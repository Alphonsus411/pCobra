...
- Transpiladores legacy retirados; se documentan solo en anexos internos o históricos.
...
### Contrato arquitectónico de paquetes `.co`

Los paquetes `.co` son contenedores ZIP con manifiesto `cobra.pkg.json`. La detección usa `zipfile` y comprueba la presencia de `cobra.pkg.json` en la raíz; los archivos `.cobra` o `.co` internos se mantienen como recursos hasta que el usuario decida ejecutarlos o editarlos explícitamente.

El soporte pertenece a `pcobra.cobra.packaging`, una capa independiente de empaquetado y distribución. No hay nuevas palabras clave ni cambios gramaticales, por lo que no se modifica `src/pcobra/core/lexer.py`, `src/pcobra/core/parser.py` ni los módulos canónicos bajo `src/pcobra/cobra/core/`.
...
Los antiguos targets retirados no forman parte del BackEnd oficial. Su detalle se conserva únicamente en anexos internos, documentos históricos y la guía de migración `docs/migracion_targets_retirados.md`.
