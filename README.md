...
La compatibilidad mínima por backend no es uniforme: `src/pcobra/cobra/transpilers/compatibility_matrix.py` declara `python` como `full` para la matriz contractual actual, mientras `javascript`, `rust`, `wasm`, `go`, `cpp`, `java` y `asm` se mantienen en `partial`. En esta revisión `javascript`, `rust` y `wasm` pasan de fallbacks puros a adaptadores oficiales mantenidos por el proyecto: JavaScript expone un objeto runtime Holobit y alias ejecutables de `corelibs`/`standard_library`, Rust genera `CobraHolobit` + `CobraRuntimeError` + funciones base inline, y WASM emite wrappers WAT con imports `pcobra:*`. Ninguno de esos tres backends debe promocionarse todavía como paridad total con `holobit_sdk`.
