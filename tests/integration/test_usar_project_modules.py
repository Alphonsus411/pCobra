...
def test_usar_desde_otro_directorio_respeta_cobra_toml(tmp_path, monkeypatch):
    ...
    (proyecto / "util.cobra").write_text(
        'variable desde_root := "ok"\n', encoding="utf-8"
    )
    app = proyecto / "app"
    app.mkdir()
    main = app / "main.cobra"
...
def test_usar_carga_una_sola_vez_con_cache_y_monkeypatch(
    crear_modulo_cobra, monkeypatch
):
...
def test_usar_detecta_ciclo_directo(crear_modulo_cobra):
    ...
    with pytest.raises(
        ImportError, match=r"Ciclo de módulos detectado en usar: .*a\.cobra"
    ):
...
def test_usar_detecta_ciclo_indirecto(crear_modulo_cobra):
    ...
    with pytest.raises(
        ImportError,
        match=r"Ciclo de módulos detectado en usar: .*a\.cobra.*b\.cobra.*c\.cobra.*a\.cobra",
    ):
        ejecutar_archivo(main)
...
def test_usar_modulo_inexistente_muestra_error_claro(crear_modulo_cobra, tmp_path):
    ...
    with pytest.raises(
        FileNotFoundError,
        match=r"Módulo de proyecto no encontrado: modulo_que_no_existe\. Ruta buscada: .*modulo_que_no_existe\.cobra",
    ):
        ejecutar_archivo(main)
...
def test_usar_bloquea_traversal_con_puntos_dobles(tmp_path):
    with pytest.raises((ValueError, PermissionError), match="traversal|inválido|fuera"):
        usar_modulo(
            "../secreto", project_root=tmp_path, current_file=tmp_path / "main.cobra"
        )


def test_usar_bloquea_escape_de_raiz_autorizada(crear_modulo_cobra, tmp_path):
    # Crear un módulo fuera de la raíz del proyecto
    (tmp_path.parent / "modulo_secreto.cobra").write_text(
        "variable secreto := 'valor_secreto'"
    )
...
def test_usar_no_soporta_sintaxis_desde(crear_modulo_cobra):
    ...
    with pytest.raises(
        ParserError,
        match="Se esperaba una ruta de módulo entre comillas o identificadores separados por puntos",
    ):
        ejecutar_archivo(main)
