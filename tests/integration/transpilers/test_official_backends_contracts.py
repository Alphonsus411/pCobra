...
def _feature_nodes(feature: str) -> list[object]:
    base = {
        ...
        "transformar": [
            NodoHolobit("hb", [1, 2, 3]),
            NodoTransformar(
                NodoIdentificador("hb"), NodoValor("rotar"), [NodoValor(90)]
            ),
        ],
        "graficar": [
            NodoHolobit("hb", [1, 2, 3]),
...
@pytest.mark.parametrize("backend", PUBLIC_BACKENDS)
def test_official_backend_generated_code_includes_expected_imports_hooks_and_adapter_markers(
    backend: str,
):
...
@pytest.mark.parametrize("backend", PUBLIC_BACKENDS)
def test_official_backend_only_injects_cobra_hooks_when_ast_requires_holobit(
    backend: str,
):
    generated = _generate(
        backend,
...
@pytest.mark.parametrize("backend", PUBLIC_BACKENDS)
def test_minimal_corelibs_standard_library_route_or_contractual_failure_is_explicit(
    backend: str,
):
    generated = _generate(
        backend,
...
def test_python_backend_generated_program_exposes_public_runtime_import_route():
    code = _generate(
        "python", [NodoAsignacion("x", NodoValor(1)), NodoAsignacion("y", NodoValor(2))]
    )
