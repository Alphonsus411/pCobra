...
def _extract_hook_signature_lines(
    generated: str, markers: tuple[str, ...]
) -> list[str]:
    ...
    for marker in markers:
        ...
        assert (
            line is not None
        ), f"No se encontró hook canónico '{marker}' en el código generado"
        extracted.append(line.rstrip())
    return extracted


@pytest.mark.parametrize("backend", OFFICIAL_TARGETS)
def test_holobit_runtime_hooks_match_golden_snapshots_for_all_official_targets(
    backend: str,
):
    ...
    assert (
        golden_path.exists()
    ), f"Falta golden de hooks para backend={backend}: {golden_path}"
