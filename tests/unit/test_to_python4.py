...
def test_transpilar_funcion():
    nodo = NodoFuncion(
        "sumar", ["a", "b"], [NodoAsignacion("resultado", NodoValor("a + b"))]
    )
    transpiler = TranspiladorPython()
    result = transpiler.generate_code([nodo])
...
def test_transpilar_clase():
    metodo = NodoMetodo(
        "mi_metodo", ["param"], [NodoAsignacion("x", NodoValor("param + 1"))]
    )
...
def test_transpilar_metodo():
    nodo = NodoMetodo(
        "mi_metodo", ["a", "b"], [NodoAsignacion("resultado", NodoValor("a + b"))]
    )
    transpiler = TranspiladorPython()
    result = transpiler.generate_code([nodo])
    expected = IMPORTS + "def mi_metodo(a, b):\n    resultado = a + b\n"
