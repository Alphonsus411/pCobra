...
def test_escalar_usa_sdk(monkeypatch):
    ...
    monkeypatch.setattr(
        "holobit_sdk.core.holobit.Holobit.escalar", fake_escalar, raising=False
    )
    h = Holobit([1, 2, 3])
    escalar(h, 2)
...
def test_mover_usa_sdk(monkeypatch):
    ...
    monkeypatch.setattr(
        "holobit_sdk.core.holobit.Holobit.mover", fake_mover, raising=False
    )
    h = Holobit([1, 2, 3])
    mover(h, 0.5, -1.0, 2.0)
...
def test_escalar_fallback(monkeypatch):
    ...
    dummy = DummyHolobit()
    monkeypatch.setattr(
        "core.holobits.transformacion._to_sdk_holobit", lambda hb: dummy
    )
