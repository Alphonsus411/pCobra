...
def test_existe_no_se_habilita_solo_por_nombre_sin_usar_archivo():
    interp = InterpretadorCobra()
    ast = generar_ast(
        'func existe(ruta) { retorno verdadero }\nimprimir(existe("README.md"))'
    )
...
def test_existe_rechaza_metadata_con_public_api_tipo_incorrecto():
    ...
    with pytest.raises(
        ...
        match="metadata usar inválida: metadata.public_api debe ser True",
    ):
        interp.ejecutar_ast(ast_llamada)
...
def test_usar_numpy_rechaza_con_error_corto():
    ...
    with pytest.raises(
        PermissionError,
        match="No se puede usar 'numpy': módulo fuera del catálogo público",
    ):
        interp.ejecutar_ast(ast)
...
def test_existe_como_metodo_sigue_bloqueado_en_modo_seguro():
    interp = InterpretadorCobra()
    ast = generar_ast(
        'obj = { existe: func(ruta) { retorno verdadero } }\nimprimir(obj.existe("README.md"))'
    )
...
def test_aceptacion_repl_runtime_datos_numero_archivo_y_politica_numpy_en_modo_seguro():
    ...
    lineas = [
        linea.strip() for linea in salida.getvalue().splitlines() if linea.strip()
    ]
    ...
    with pytest.raises(
        PermissionError,
        match=r"(modulo_fuera_catalogo_publico|módulo fuera del catálogo público)",
    ):
        interp.ejecutar_ast(generar_ast('usar "numpy"'))
...
def test_metadata_usar_error_mismatch_payload_incluye_detalle_simbolo_tipo_y_clave():
    ...
    interp._validador._metadata_simbolos_usar["existe"] = dict(
        interp._validador._metadata_simbolos_usar["existe"]
    )
