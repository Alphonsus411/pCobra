...
def test_cli_wrapper_targets_iguala_contrato_canonico() -> None:
    assert (
        cli_registry.cli_transpiler_targets()
...
def test_cli_wrapper_carga_entrypoints_por_contrato_de_get_transpilers(
    monkeypatch,
) -> None:
    calls = {"ensure": 0}
    ...
    monkeypatch.setattr(
        canonical_registry, "ensure_entrypoint_transpilers_loaded_once", _fake_ensure
    )
...
def test_contrato_publico_transpiladores_conserva_solo_targets_oficiales_en_orden() -> (
    None
):
