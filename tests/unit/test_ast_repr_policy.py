...
def _decorador_dataclass(nodo_clase: ast.ClassDef) -> ast.Call | ast.Name | None:
    for decorador in nodo_clase.decorator_list:
        ...
        if (
            ...
            and decorador.func.id == "dataclass"
        ):
            return decorador
    return None
...
def test_politica_repr_nodos_ast() -> None:
    ...
    clases_nodo = [
        ...
        and nodo.name.startswith("Nodo")
        and nodo.name != "NodoAST"
    ]
    ...
    for clase in clases_nodo:
        ...
        if metodo_str is not None:
            texto_str = (
                ast.get_source_segment(
                    ARCHIVO_AST_NODES.read_text(encoding="utf-8"), metodo_str
                )
                or ""
            )
            assert (
                "__repr__" in texto_str
            ), f"{clase.name}.__str__ debe delegar a __repr__"
