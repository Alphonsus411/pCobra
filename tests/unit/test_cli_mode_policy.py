...
def test_modo_cobra_bloquea_verificar(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        verify_module, "mostrar_error", lambda msg: mensajes.append(msg)
    )
...
def test_modo_cobra_bloquea_transpilar_inverso(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        inverse_module, "mostrar_error", lambda msg: mensajes.append(msg)
    )

    args = SimpleNamespace(
        archivo=str(archivo), origen="python", destino="python", modo="cobra"
    )
...
def test_modo_cobra_bloquea_compilar(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        compile_module, "mostrar_error", lambda msg: mensajes.append(msg)
    )

    args = SimpleNamespace(
        archivo=str(archivo), tipo="python", backend=None, tipos=None, modo="cobra"
    )
...
def test_modo_transpilar_bloquea_ejecutar(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        execute_module, "mostrar_error", lambda msg: mensajes.append(msg)
    )

    args = SimpleNamespace(
        archivo=str(archivo), sandbox=False, contenedor=None, modo="transpilar"
    )
...
def test_modo_mixto_permite_ejecutar_y_falla_por_archivo(monkeypatch, tmp_path):
    ...
    args = SimpleNamespace(
        archivo=str(archivo), sandbox=False, contenedor=None, modo="mixto"
    )
    rc = ExecuteCommand().run(args)
...
def test_modo_transpilar_permite_compilar_y_falla_por_archivo(monkeypatch, tmp_path):
    ...
    args = SimpleNamespace(
        archivo=str(archivo), tipo="python", backend=None, tipos=None, modo="transpilar"
    )
    rc = CompileCommand().run(args)
...
def test_menu_modo_cobra_muestra_solo_ejecutar(monkeypatch):
    ...
    monkeypatch.setattr(
        "cobra.cli.cli.sys.stdin", types.SimpleNamespace(isatty=lambda: True)
    )
    monkeypatch.setattr(
        "cobra.cli.cli.messages.mostrar_info", lambda msg: mensajes_info.append(msg)
    )
    monkeypatch.setattr("builtins.input", lambda _: "archivo.cobra")
...
def test_menu_modo_transpilar_muestra_solo_transpilar(monkeypatch):
    ...
    respuestas = iter(["s", "archivo.cobra", "python"])
    monkeypatch.setattr("builtins.input", lambda _: next(respuestas))
    monkeypatch.setattr(
        "cobra.cli.commands.execute_cmd.ExecuteCommand.run",
        lambda *_: llamadas_ejecutar.__setitem__(
            "count", llamadas_ejecutar["count"] + 1
        ),
    )

    rc = app.execute_command(
        argparse.Namespace(cmd="menu", modo="transpilar", lang="es")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            verify_module,
            VerifyCommand(),
            SimpleNamespace(archivo="demo.cobra", lenguajes=["python"], modo="cobra"),
        ),
        (
            inverse_module,
            TranspilarInversoCommand(),
            SimpleNamespace(
                archivo="demo.py", origen="python", destino="javascript", modo="cobra"
            ),
        ),
        (
            validar_sintaxis_module,
...
def test_comandos_codegen_bloqueados_muestran_mensaje_homogeneo(
    module, command, args, monkeypatch
):
    mensajes = []
    ...
    assert rc == 1
    assert any(
        "Acción sugerida: cambia a --modo mixto o --modo transpilar" in m
        for m in mensajes
    )
